...
// 4) 内部リンク [/gajumaru-info/PATH/] の PATH が実在する記事ID（または about/privacy）であること
import fs from 'fs';
...
const expected = ['basics', 'strangler', 'varieties', 'growing', 'propagation', 'troubles', 'pet-safety', 'folklore', 'fengshui', 'faq'];
...
for (const { id, block } of blocks) {
  ...
  for (const lk of [...content.matchAll(/\]\(\/gajumaru-info\/([a-z0-9-]+)\//g)]) {
    const target = lk[1];
